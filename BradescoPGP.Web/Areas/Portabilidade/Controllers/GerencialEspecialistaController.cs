...
{
    public class GerencialEspecialistaController : AbstractController
    {
        private ISolicitacaoService _solicitacaoService;
        private readonly IUtil _util;
        private readonly PGPEntities _context;
        public GerencialEspecialistaController(DbContext context, ISolicitacaoService solicitacaoService, IUtil util) : base(context)
        {
        ...
        // GET: Portabilidade/GerencialEspecialista
        public ActionResult Index(FiltrosPortabilidade filtros)
        {
            ViewBag.Titulo = "Especialista";
            ...
            foreach (var motivo in motivos)
            {
                ...
                var idSubStatus = substatus.Where(s => s.Descricao.StartsWith("Reti") || s.Descricao.StartsWith("Não Reti"))
            ...
            var equipesDict = _context.Usuario
                .GroupBy(g => g.Matricula)
                .ToDictionary(k => k.Key, v => v.FirstOrDefault().Equipe);

            var equipes = _context.Usuario.Where(s => s.Perfil.Descricao == NivelAcesso.Especialista.ToString()).Select(s => s.Equipe).Distinct().ToList();
            ...
            var viewModel = new EspecialistaViewModel
            {
                NomeEspecialista = dados.Where(s => ViewBag.Especialistas).ToString(),
        ...
        public JsonResult TabelaStatus(string status, string substatus, FiltrosPortabilidade filtros = null)
        {
            ...
            var resumo = Resumo(model);

            var dados = new {data = model, resumo};
        ...
        #region MÉTODOS DE OBTER DADOS PRAS TABELAS
        ...
        private List<Solicitacao> ObterEquipe(string serie, string equipe, FiltrosPortabilidade filtros = null)
        {
            ...
            if (serie.ToLower().StartsWith("retido"))
            {
                ...
                dados = dados.Where(s => s.SubStatusId == substatus.FirstOrDefault(x => x.Descricao.StartsWith("Ret")).Id).ToList();

            }
            else if (serie.ToLower().StartsWith("não"))
            {
                dados = dados.Where(s => s.SubStatusId == substatus.FirstOrDefault(x => x.Descricao.StartsWith("Não Ret")).Id).ToList();
            }
            return dados;
        }
        ...
        public ActionResult ExpertarExcel(TipoExportacaoExcelGrafico tipoPesquisa,
            ParamentrosExportacaoExcelGraficos parametro, FiltrosPortabilidade filtros = null)
        {
            var dados = new List<Solicitacao>();
        ...
        #region Resumo
        private object Resumo(List<TabelaStatusViewModel> model)
        {
            var quantidadeRetido = model.Count(s => s.ValorRetido.HasValue);

            var valorRetido = model.Where(s => s.ValorRetido.HasValue).Sum(s => s.ValorRetido ?? 0).ToString("n2", new CultureInfo("pt-BR"));

            var valorNaoRetido = model.Where(s => !string.IsNullOrEmpty(s.SubStatus) && s.SubStatus.StartsWith("Não Reti")).Sum(s => s.ValorPrevistoSaida).ToString("n2", new CultureInfo("pt-BR"));

            var quantidadeNaoRetido = model.Count(s => !string.IsNullOrEmpty(s.SubStatus) && s.SubStatus.Contains("Não Reti"));
        ...
        {
            return string.IsNullOrEmpty(filtros.Especialista) || filtros.De.HasValue || filtros.Ate.HasValue;
        }
    }
}